window.GameUI = (()=>{
  ...
  function boardHTML(data,state,clickable=true){const r=data.rounds[state.round];let h='<div class="grid">';r.categories.forEach(c=>h+=`<div class="cat">${esc(c.name)}</div>`);for(let qi=0;qi<5;qi++){r.categories.forEach((c,ci)=>{const q=c.questions[qi],key=`${state.round}:${ci}:${qi}`,used=state.used[key];h+=`<button class="tile ${used?'used':''}" data-ci="${ci}" data-qi="${qi}" ${(!clickable||used)?'disabled':''}>${q.value}</button>`})}return h+'</div>'}
